...
from .utils import logger, save_audio


def cmd_show(grid_path: Path) -> None:
    """Display the grid as ASCII score."""
...
def cmd_set(
    ...
    """Add or update an event at a grid position."""
    grid = EventGrid.from_json(grid_path)

    drum_role = DrumRole(role)

    # Remove existing event at same position+role
...
def cmd_remove(
    ...
    """Remove events at a grid position."""
    grid = EventGrid.from_json(grid_path)
...
def cmd_velocity(
    ...
    """Set or scale velocity for events, optionally filtered by role."""
    grid = EventGrid.from_json(grid_path)
...
def cmd_render(
    ...
    """Render event grid to audio file."""
    grid = EventGrid.from_json(grid_path)
...
def cmd_export_midi(grid_path: Path, output_path: Path) -> None:
    """Export event grid to MIDI file."""
    grid = EventGrid.from_json(grid_path)
...
def cmd_generate(
    ...
    """Generate a new skeleton pattern and save as event_grid.json."""
