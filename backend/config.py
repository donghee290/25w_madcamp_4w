...
def load_config() -> AppConfig:
    ...
    upload_root = Path(
        os.getenv("SOUNDROUTINE_UPLOAD_ROOT", str(data_root / "jobs"))
    )
    output_root = Path(
        os.getenv("SOUNDROUTINE_OUTPUT_ROOT", str(data_root / "jobs"))
    )
    role_config_path = Path(
        os.getenv(
    ...
    mongo_collection = os.getenv(
        "SOUNDROUTINE_MONGO_COLLECTION",
        os.getenv("MONGO_COLLECTION", "jobs"),
    )
