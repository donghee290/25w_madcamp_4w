...
def create_app(config: Optional[AppConfig] = None) -> Flask:
    ...
    def create_beat() -> Any:
        ...
        if content_type.startswith("multipart/form-data"):
            ...
            upload_candidates = []
            upload_candidates.extend(request.files.getlist("files"))
            if "file" in request.files:
                upload_candidates.append(request.files["file"])
        ...
        try:
            ...
            # Render audio + build grid/events via beat service
            beat_service = get_beat_service()
            output_rel = f"{job_id}/outputs/render.wav"
            result = beat_service.generate_beat(
                pools_data=pools_json,
                bpm=bpm,
                bars=bars,
                seed=seed,
                motion_mode=motion_mode,
                ...
                fill_prob=fill_prob,
                texture_enabled=texture_enabled,
                output_filename=output_rel,
                sample_root=uploads_dir,
            )

            grid_json = result.get("grid", {})
            events_json = result.get("events", [])
            chosen = result.get("chosen_samples", {})
    ...
    @app.route("/v1/beats", methods=["GET"])
    def list_beats() -> Any:
